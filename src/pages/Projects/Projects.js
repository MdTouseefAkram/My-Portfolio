import React from "react";
import "./Projects.css";
import HeadShake from "react-reveal/HeadShake";
import WeatherApp from "../../assets/Weather App.png";
import Portfolio from "../../assets/Portfolio.png";
import Razorpay from "../../assets/Razorpay.png";
import Automatic_Attendance_System from "../../assets/Automatic Attendance System.png";
import { FaLink } from "react-icons/fa6";
import { FaGithub } from "react-icons/fa";
const Projects = () => {
  return (
    <>
      <div className="continer project" id="projects">
        <h2 className="col-12 mt-3 mb-1 text-center text-uppercase">
          TOP RECENT PROJECTS
        </h2>
        <hr />
        <h6 className="pb-3 text-center">
          ✨ Thrilled to present my projects! ✨
        </h6>
        {/* card design */}
        <div className="row" id="ads">
          <HeadShake>
            {/* Project 1 */}

            <div className="col-md-6">
              <div className="card rounded">
                <div className="card-image">
                  <span className="card-notify-badge">Front-End</span>
                  <img src={WeatherApp} alt="project1" />
                </div>
                <div className="card-image-overly m-auto mt-3">
                  <span className="card-detail-badge">HTML</span>

                  <span className="card-detail-badge">CSS</span>
                  <span className="card-detail-badge">JavaScript</span>
                  <span className="card-detail-badge">API</span>
                </div>
                <div className="card-body text-center">
                  <div className="ad-title m-auto">
                    <h5>Weather App</h5>
                  </div>
                  <div className="ad-title m-auto">
                    <p className="desc">
                      Designed and developed a web application that provides
                      real-time weather information for a given location. The
                      app uses free weather API to retrieve weather data and
                      displays it in an easy-to-read format.
                    </p>
                  </div>
                  <div className="Link-github">
                    <a
                      className="ad-btn"
                      href="https://mdtouseefakram.github.io/Weather-App/"
                      target="_blank"
                      rel="noreferrer"
                    >
                      Live
                      <FaLink className="icons" size={20} />
                    </a>
                    <a
                      className="ad-btn"
                      href="https://github.com/MdTouseefAkram/Weather-App"
                      target="_blank"
                      rel="noreferrer"
                    >
                      GitHub
                      <FaGithub className="icons" size={20} />
                    </a>
                  </div>
                </div>
              </div>
            </div>

            {/* Project 2 */}
            <div className="col-md-6">
              <div className="card rounded">
                <div className="card-image">
                  <span className="card-notify-badge">Front-End</span>
                  <img src={Portfolio} alt="project2" />
                </div>
                <div className="card-image-overly m-auto mt-3">
                  <span className="card-detail-badge">ReactJS</span>
                  <span className="card-detail-badge">Bootstrap</span>
                </div>
                <div className="card-body text-center">
                  <div className="ad-title m-auto">
                    <h5>Portfolio App</h5>
                  </div>
                  <div className="ad-title m-auto">
                    <p className="desc">
                      Developed a personal portfolio web app using React.js to
                      showcase projects, skills, and resume. Designed a
                      responsive and animated UI, ensuring a visually appealing
                      and seamless user experience.
                    </p>
                  </div>
                  <div className="Link-github">
                    <a
                      className="ad-btn"
                      href="https://my-portfolio-website-uvz6.onrender.com/"
                      target="_blank"
                      rel="noreferrer"
                    >
                      Live
                      <FaLink className="icons" size={20} />
                    </a>
                    <a
                      className="ad-btn"
                      href="https://github.com/MdTouseefAkram/My-Portfolio"
                      target="_blank"
                      rel="noreferrer"
                    >
                      GitHub
                      <FaGithub className="icons" size={20} />
                    </a>
                  </div>
                </div>
              </div>
            </div>

            {/* Project 3 */}
            <div className="col-md-6 mb-5">
              <div className="card rounded">
                <div className="card-image">
                  <span className="card-notify-badge">Front-End</span>
                  <img src={Razorpay} alt="project3" />
                </div>
                <div className="card-image-overly m-auto mt-3">
                  <span className="card-detail-badge">HTML</span>
                  <span className="card-detail-badge">Tailwind CSS</span>
                  <span className="card-detail-badge">Animations</span>
                </div>
                <div className="card-body text-center">
                  <div className="ad-title m-auto">
                    <h5>Razorpay Clone</h5>
                  </div>
                  <div className="ad-title m-auto">
                    <p className="desc">
                      Developed a fully responsive Razorpay clone with a modern
                      UI, closely replicating the original design using HTML and
                      Tailwind CSS.
                    </p>
                  </div>
                  <div className="Link-github">
                    <a
                      className="ad-btn"
                      href="https://taupe-buttercream-ff09dd.netlify.app/"
                      target="_blank"
                      rel="noreferrer"
                    >
                      Live
                      <FaLink className="icons" size={20} />
                    </a>
                    <a
                      className="ad-btn"
                      href="https://github.com/MdTouseefAkram/Razorpay-Clone"
                      target="_blank"
                      rel="noreferrer"
                    >
                      GitHub
                      <FaGithub className="icons" size={20} />
                    </a>
                  </div>
                </div>
              </div>
            </div>

            {/* Project 4 */}
            <div className="col-md-6 mb-5">
              <div className="card rounded">
                <div className="card-image">
                  <span className="card-notify-badge">Machine Learning</span>
                  <img src={Automatic_Attendance_System} alt="project4" />
                </div>
                <div className="card-image-overly m-auto mt-3">
                  <span className="card-detail-badge">Python</span>
                  <span className="card-detail-badge">OpenCV</span>
                  <span className="card-detail-badge">Tkinter GUI</span>
                  <span className="card-detail-badge">MySQL</span>
                </div>
                <div className="card-body text-center">
                  <div className="ad-title m-auto">
                    <h5>Face Recognition Automatic Attendance System</h5>
                  </div>
                  <div className="ad-title m-auto">
                    <p className="desc">
                      Developed a Face Recognition Automatic Attendance System
                      using Python, OpenCV, Tkinter GUI and MySQL. Implemented
                      real-time face detection and recognition using Haar
                      Cascade and LBPH algorithms and generates attendance
                      report in Excel file.
                    </p>
                  </div>
                  <div className="Link-github">
                    <a
                      className="ad-btn"
                      href="https://github.com/MdTouseefAkram/Face-Recognition-Automatic-Attendance-System"
                      target="_blank"
                      rel="noreferrer"
                    >
                      GitHub
                      <FaGithub className="icons" size={20} />
                    </a>
                  </div>
                </div>
              </div>
            </div>
          </HeadShake>
        </div>
      </div>
    </>
  );
};

export default Projects;
